$(function(){
    ...
    var RightMenuView = Backbone.View.extend({
        tagName: "div",
        className: "rightMenu",
        template: _.template($("#rightMenu-template").html()),

        events:{
            "mousedown .shortcutTray" : "showMenu"
        },
    ...
    var AppView = Backbone.View.extend({
        ...
        initialize: function() {
            ...
            menus.fetch({
                url : window._contextPath + "/sys/menu/treeData",
                success: function(collection, response, options){
                    //collection.each(function(menu) {
                    //    console.log(menu.get("href"))
                    //});
                    app.addAll();
                    app.adjustPosition()
                }
        ...
        addRightMenu: function(){
            ...
            $("#main").append(rightMenuView.render().el)
        },
        addSign: function(){
            var signView = new SignView();
            $(".pluginGroup").append(signView.render().el)
            var notifyView = new SignView();
            $(".pluginGroup").append(notifyView.render().el)
        },

        addOne: function(menu) {
            var menuView = new MenuView({model: menu});
            this.$el.find("#desk").append(menuView.render().el);
        },
